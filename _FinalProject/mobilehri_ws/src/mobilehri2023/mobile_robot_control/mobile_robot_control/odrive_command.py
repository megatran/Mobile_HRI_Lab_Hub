...
class odrive_command(Node):
    def __init__(self):
        ...
        # self.get_logger().info(str(self.odrv.axis0.error)+' '+str(self.odrv.axis0.error))
        # self.odrv.reboot()
        #
        # self.odrv = odrive.find_any()
        ...
        self.axis0.config.watchdog_timeout = 10
        self.axis1.config.watchdog_timeout = 10
        ...
        self.twist_sub = self.create_subscription(Twist, "/cmd_vel", self.diff_drive_callback, 10) # original code 
        self.vision_sub = self.create_subscription(Twist, "/cmd_vision", self.vision_drive_callback, 10) 
        self.joy_sub = self.create_subscription(Joy, '/joy', self.joyCallback, 10)

        self.twist_pub = self.create_publisher(Twist,'/odrv_VlVr',10)
    ...
    def joyCallback(self,msg):
        ...
        if msg.buttons[4] == 1.0: # when L1 pressed
            self.use_vision = False
        elif msg.buttons[5]==1.0: # when R1 pressed (use-vision)
            self.use_vision = True

        else:
            self.use_vision = False
        if msg.buttons[4] == 1.0 and msg.buttons[5] == 1.0 :
            self.use_vision = False

    def vision_drive_callback(self, msg):
        if self.use_vision is None or not self.use_vision :
            return 
        v = msg.linear.x
        w = msg.angular.z
        w = -0.2*w
        v = 0.5*v
        # TODO: Your Code Here -------------------------------------------
        Vl = (v-self.wheel_track*0.5*w)/self.R
        Vr = (v+self.wheel_track*0.5*w)/self.R
        ...
        if abs(Vl) <= 0.08:
            ...
            self.axis0.requested_state = AXIS_STATE_CLOSED_LOOP_CONTROL
            self.axis0.controller.input_vel = -1*Vl
        ...
        if abs(Vr) <= 0.08:
            ...
            self.axis1.requested_state = AXIS_STATE_CLOSED_LOOP_CONTROL
            self.axis1.controller.input_vel = Vr
        t=Twist()
        t.linear.x = Vl
        t.linear.y=Vr
    ...
    def diff_drive_callback(self, msg):
        if self.use_vision is None or self.use_vision:
            return
        v = msg.linear.x
        w = msg.angular.z
        ...
        self.twist_pub.publish(t)
        return
######################################### Follow Me Code (Frank) #################################
 

    


######################################### Follow Me Code (Frank- End) #################################
